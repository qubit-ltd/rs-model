...
fn test_claim_statuses_preserve_source_groups() {
    let individual_status_group: Box<dyn Fn(InsuranceClaimStatus) -> InsuranceClaimStatusGroup> =
        Box::new(InsuranceClaimStatus::status_group);
    let enterprise_status_group: Box<dyn Fn(EnterpriseClaimStatus) -> EnterpriseClaimStatusGroup> =
        Box::new(EnterpriseClaimStatus::status_group);
    assert_eq!(
        individual_status_group(InsuranceClaimStatus::ClaimApplicationWaitAudit),
        InsuranceClaimStatusGroup::PendingCase
    );
    assert_eq!(
        individual_status_group(InsuranceClaimStatus::InsuranceCompanyCompleted),
    ...
    assert_eq!(
        enterprise_status_group(EnterpriseClaimStatus::ClaimApplicationWaitAudit),
        EnterpriseClaimStatusGroup::Register
    );
    assert_eq!(
        enterprise_status_group(EnterpriseClaimStatus::InsuranceCompanyCompleted),
